...
- `rotp.mp.client` — `NetClient` (WebSocket), `ClientMain` (with a Mac-style menu bar wiring the spec's ⌘-shortcuts, e.g. ⌘T Technology, ⌘N Next Turn), and DTO-rendered screens: `GalaxyViewPanel` (clickable galaxy map), `ColonyPanel` (colony management), `ResearchPanel` (research, opens as its own window), `ColonyAllocations` (pure spending/research-redistribution logic, shared). More screens added incrementally.
