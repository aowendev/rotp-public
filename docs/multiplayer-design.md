...
- **Phase 1.5 — human-validated full playthrough: DONE (2026-08-08).** A human played the economy→research→build→expand→diplomacy loop across every core screen and the gaps found were fixed live. See the handoff doc for the batch.
- **Phase 2 — LAN & session completeness: DONE.** Lobby AI-fill / solo-vs-AI, client reconnection, minimal save/load, and race / galaxy-size / AI-ability lobby picks. Player colour and the galaxy-size option set were deliberately pushed to the browser client.
- **Phase 3 — interactive mid-turn prompts: DONE.** Seven increments — tech selection, incoming diplomacy, council vote, colonize choice, turn timers, public GNN news, and combat/spy alerts — on a reusable `Prompts` envelope.
- **Phase 4 — ALL game functionality on the wire: functionally COMPLETE (2026-08-10).** Redefined (user, 2026-08-09) from "internet hosting" to *every decision a desktop player can make must be reachable over the protocol* — because Phase 5 is purely a client, so any bug found there must be conclusively a client bug. Landed: reserve transfers (both directions), session-token reconnection, contact-via-war, council-vote persistence, fuller diplomacy (tech exchange with counter-offers, aid, threats, joint war), per-empire multi-human outcomes, AI takeover for dropped players, the bombard / steal-tech / sabotage choices, server error-hardening, and a published CC0 protocol specification. **Ship combat auto-resolves by decision** — an interactive battle would stall every other player — so only the decisions around it are on the wire. Validation of all this is **Phase 4.5**. **Hosting moved to Phase 6.**
- **Phase 4.5 — manual testing and verification: NEXT (current focus).** Everything in
  Phase 4 is proven only by in-process integration tests, where latency is zero, both
  clients share a JVM, and a galaxy is whatever the unseeded RNG produced. Phase 4.5 is
  a person, two machines, and **`docs/mp-test-scenarios.md`** — the checklist that
  decides whether Phase 4 was actually finished. Mirrors Phase 1.5, which did the same
  job for single-player and found a batch of gaps no test had. Exit criteria: the
  checklist passes end to end, the server survives a session of deliberate abuse with no
  stack traces, and anything it turns up is fixed (fixes are Phase-4 work; finding them
  is this phase). Do not start Phase 5 on an unvalidated backend — the whole point of
  the sign-off is that a bug found there is conclusively a client bug.
- **Phase 5 — browser client, in a SEPARATE PRIVATE REPO** (not under this licence; it speaks the CC0 protocol in `docs/protocol.md`)** speaking the identical protocol (the DTO client screens are its blueprint), reproducing the **1990s Mac-port interaction feel and keyboard shortcuts** (see §1; source the specifics first — don't invent them).
- **Phase 6 — hosting and a game launcher:** deploy the server (Scaleway for initial testing — note their VMs are not free; see `docs/deployment.md`), plus a front end that spins up a JVM per game for players to join. Measured at ~128MB per concurrent game.
