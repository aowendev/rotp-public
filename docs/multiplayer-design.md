...
- `rotp.mp.client` — `NetClient` (WebSocket), `ClientMain` (with a Mac-style menu bar wiring the spec's ⌘-shortcuts, e.g. ⌘F Fleet List, ⌘T Technology, ⌘N Next Turn), and DTO-rendered screens: `GalaxyViewPanel` (clickable galaxy map), `ColonyPanel` (colony management), `ResearchPanel` (research), `FleetsPanel` (fleets & transports) — each opening as its own window — plus pure helpers `ColonyAllocations` (spending/research redistribution) and `FleetView` (fleet summarization/deployability). More screens added incrementally.
